...
| 機能名                                   | 機能概要                                                                                                       |
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| TBL出力（設備データ管理マスタテーブル→2D用最終断面テーブル） | 設備データ管理マスタテーブルから設備データを取得し、編集したものを2D用最終断面テーブル（マテリアライズドビュー）に登録し、GeoServerに配信設定を登録する。 |
...
5. Pythonスクリプトを実行します。
    ```bash
    cd App/functions
    python FDI_0090_tblExportEquipmentMasterTo2d.py --layer_id=<レイヤID>
    ```
...
6. 実行ログを確認します。
    ...
    ```
    [2026-02-03 18:54:58,621] [INFO] [FDI] BPI0001:FDI_0090_TBL出力（設備データ管理マスタテーブル→2D用最終断面テーブル）処理 開始
    [2026-02-03 18:54:58,710] [INFO] [FDI] BPI0002:FDI_0090_TBL出力（設備データ管理マスタテーブル→2D用最終断面テーブル）処理 終了
    ```
